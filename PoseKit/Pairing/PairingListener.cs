...
public sealed class PairingListener : IDisposable
{
    ...
    /// Raised when a "posekitqueue" readiness tell arrives from the currently-paired peer —
    /// CoupleQueueService reacts to this locally; nothing is ever sent back in response to it.
    public event Action<PartnerIdentity>? QueueSignalReceived;
    ...
    private void OnChatMessage(IHandleableChatMessage message)
    {
        ...
        if (text.Equals(QueueKeyword, StringComparison.OrdinalIgnoreCase))
        {
            if (state.Active && state.Peer is { } peer && peer.Equals(sender))
                QueueSignalReceived?.Invoke(sender);
        }
    }
